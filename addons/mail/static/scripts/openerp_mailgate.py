...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the Eagle ERP server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="Eagle ERP user id to connect with",
                      default=config.OPENERP_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="Eagle ERP user password",
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="Eagle ERP database name (default: %default)",
                      default=config.OPENERP_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="Eagle ERP Server hostname",
                      default=config.OPENERP_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="Eagle ERP Server XML-RPC port number",
...
def main():
    """
    Receive the email via the stdin and send it to the Eagle ERP Server
    """
    ...
    try:
        ...
        subject = '[Eagle ERP]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to Eagle ERP Server, sending error notification to %s\n" % config.MAIL_ADMINS)
